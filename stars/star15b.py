...
class Creature(object):
    base_damage_per_hit = 3

    def __init__(self, y, x, HP=200, damage_per_hit=None):
        if not damage_per_hit:
            damage_per_hit = self.base_damage_per_hit
        self.HP = HP
...
class Cavern(object):
    def __init__(self, creatures=None, points=None):
        ...
        self.starting_locations = [(c.location, c.side) for c in self.creatures]
        self.points = points or set()

    def army(self, side):
        return [c for c in self.creatures if c.side == side]

    def battle_over(self):
        return not all(len(self.army(side)) for side in CreatureType)
        ...
        for y, line in enumerate(inp):
            for x, char in enumerate(line):
                ...
                if char == '.':
                    ...
                    self.starting_locations.append((p, CreatureType.goblin))
                elif char == 'E':
                    self.points.add(p)
                    self.creatures.append(Elf(y, x))
                    self.starting_locations.append((p, CreatureType.elf))

    def reset(self):
        self.creatures = [
            Elf(p.y, p.x) for p, side in self.starting_locations
            if side == CreatureType.elf
        ] + [
            Goblin(p.y, p.x) for p, side in self.starting_locations
            if side == CreatureType.goblin
        ]
    ...
    def battle(self):
        ...
        elf_count = len(self.army(CreatureType.elf))
        while elf_count == len(self.army(CreatureType.elf)):
            battle_finished = self.execute_round()
            if battle_finished:
                break
            rnd += 1
        if elf_count == len(self.army(CreatureType.elf)):
...
def do_the_thing(filename):
    ...
    while not battle_result:
        cavern.reset()
        battle_result = cavern.battle()
        print(Elf.base_damage_per_hit)
        Elf.base_damage_per_hit += 1
    print(battle_result)

if __name__ == '__main__':
    # do_the_thing('input/day15-test1.txt')
    # do_the_thing('input/day15-test3.txt')
